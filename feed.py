...
from cryptofeed.exchanges import (Binance, Bitfinex, Gateio, Huobi, OKX, BinanceFutures, GateioFutures, HuobiSwap)
...
class DataHandler:
    ...
    async def handle_book(self, book, receipt_timestamp):
        ...
        book_tuple = (
            ...
            book.exchange,
            format(book.book.bids.index(0)[0], '.4f'),
            format(book.book.bids.index(0)[1], '.4f'),
            format(book.book.asks.index(0)[0], '.4f'),
            format(book.book.asks.index(0)[1], '.4f')
        )
...
def main():
    ...
    f.add_feed(BitDotCom(**common_cfg))
    ...
    f.add_feed(BitDotCom(**common_cfg_futures))
